...
# do we have -o?
fail 'Please specify an output file with -o' unless !settings[:outfile].empty?

# do we have -d?
fail 'Please specify fragments directory with -d' unless !settings[:workdir].empty?

# can we write to -o?
if File.file?(settings[:outfile])
  if !File.writable?(settings[:outfile])
    fail "Cannot write to #{settings[:outfile]}"
  end
else
  if !File.writable?(File.dirname(settings[:outfile]))
    fail "Cannot write to dirname #{File.dirname(settings[:outfile])} to create #{settings[:outfile]}"
  end
end
...
if !File.directory?(File.join(settings[:workdir], 'fragments')) && !File.executable?(File.join(settings[:workdir], 'fragments'))
  fail 'Cannot access the fragments directory'
end
...
open('fragments.concat', 'a') do |f|
  Dir.entries('fragments').sort.each do |entry|
